	...
	interface and the Systick. */
	 ...
	 while (1) {
		 ...
		 interface and the Systick. */
		  ...
		  MX_GPIO_Init();
		  /* USER CODE BEGIN 2 */
		  uint8_t led_mask = 0x07;
		  uint8_t led_mask1 = 0x07;
		  uint8_t led_mask2 = 0x1F;
		  uint8_t led_mask3 = 0x1;
		  int bit_count = 0;
		  ...
		  switch(count){
		  case 0:
			  update_state_downcount = 100;
			  led_mask1 = led_mask1 >> 1 | led_mask1 << 7;
			  led_mask = led_mask1;
		  	  break;
		  case 1:
			  update_state_downcount = 200;
			  led_mask2 = led_mask2 << 1 | led_mask2 >> 7;
			  led_mask = led_mask2;
			  break;
		  case 2:
			  update_state_downcount = 300;
			  if(bit_count < 9){
				  led_mask3 = led_mask3 | 0x01 << bit_count;
			  }
			  else{
				  led_mask3 = led_mask3 ^ 0x01 << (bit_count - 9);
			  }
			  bit_count = (bit_count + 1) % 17;
			  led_mask = led_mask3;
			  break;
		  }
